...
/-
This section show that `Hom A A`, for `A : Mon C` a commutative object, has a
semiring structure.
-/
...
def Mon.Hom.zero_hom (G₀ H₀ : Mon 𝒞) [IsCommMonObj H₀.X] : (Hom G₀ H₀) where
  hom := (toUnit G₀.X) ≫ (MonObj.one : 𝟙_ 𝒞 ⟶ H₀.X)
...
/-
If `G` is a commutative monoidal object in a Cartesian Monoidal Category, the monoidal
homomorphisms between them have a natural semiring structure.
-/
...
instance [IsCommMonObj G.X] : Semiring (Hom G G) where
  ...
  zero_mul f := by
    ext
    show f.hom ≫ ((toUnit G.X) ≫ (MonObj.one : 𝟙_ 𝒞 ⟶ G.X)) = ((toUnit G.X) ≫ (
      MonObj.one : 𝟙_ 𝒞 ⟶ G.X))
    simp only [comp_toUnit_assoc]
  mul_zero f := by
    ext
    show ((toUnit G.X) ≫ (MonObj.one : 𝟙_ 𝒞 ⟶ G.X)) ≫ f.hom = ((toUnit G.X) ≫
      (MonObj.one : 𝟙_ 𝒞 ⟶ G.X))
